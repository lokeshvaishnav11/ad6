...
export type BalanceState = {
  balance: { balance: number; exposer: number ; commision :number }
  hide: HideBalExp
}
...
const balanceSlice = createSlice({
  ...
  initialState,
  reducers: {
    setBalance: (state, action: PayloadAction<{ balance: number; exposer: number;commision:number }>) => {
    ...
    setCom: (state, action: PayloadAction<number>) => {
      state.balance = { ...state.balance, commision: action.payload }
    },
...
export const { setBalance, setExposer, hideBalExp, setSingleBal,setCom } = balanceSlice.actions
